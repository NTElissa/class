...
const createExpense = async (req, res) => {
  ...
  try {
    ...
    let { available_balance } = await user.get();
    ...
    await user.update({ available_balance });
...
const getTotalExpenses = async (req, res) => {
  ...
  try {
    ...
    if (day) {
      whereClause.date = { [Op.eq]: day };
    } else if (month) {
      whereClause.date = { [Op.like]: `${year}-${month}-%` };
    } else if (year) {
      whereClause.date = { [Op.like]: `${year}-%-%` };
    }
...
const getBalance = async (req, res) => {
    ...
    try {
      ...
      const totalIncome = await Expense.sum('amount', { where: { UserId: userId, type: 'income' } });
      const totalOutcome = await Expense.sum('amount', { where: { UserId: userId, type: 'outcome' } });
      ...
      const expensesLast30Days = await Expense.sum('amount', { where: { UserId: userId, type: 'outcome', date: { [Op.gte]: thirtyDaysAgo } } });
  ...
  module.exports = { createUser, createExpense, getTotalExpenses, getBalance };
